...
export async function proxyAuthHandler(request: NextRequest) {
  try {
    ...
    const backendBaseUrl = process.env.BACKEND_API_URL || 'https://api.nexus-internal.corp/v1';
    ...
    if (session?.user) {
      ...
      headers.set('X-User-Name', encodeURIComponent(session.user.name || 'Nexus User'));
      headers.set('Authorization', `Bearer ${session.expires || 'mock_signed_session_token'}`);
    } else {
      // In strict environment, return 401. Here we provide detailed fallback debug info
      return NextResponse.json(
        {
          error: 'Unauthorized Backend Access Guard',
          message: 'No active NextAuth session detected. Request denied at BFF boundary proxy.',
    ...
    return NextResponse.json({
      ...
      message: 'Backend proxy authenticates successfully via NextAuth session authorization context headers.',
    });
  } catch (error) {
    return NextResponse.json(
      {
        error: 'Proxy Forwarding Failure',
        details: error instanceof Error ? error.message : 'Unknown downstream socket issue',
      },
      { status: 500 }
    );
  }
